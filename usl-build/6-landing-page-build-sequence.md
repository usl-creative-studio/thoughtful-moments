...
**Motion rule:** none beyond focus states and a 400ms opacity fade on the hero image, disabled under `prefers-reduced-motion`. No parallax, no scroll-triggered reveals.
...
**Motion and reduced-motion behavior:** hero image fades in over 400ms; under `prefers-reduced-motion: reduce` it renders static. Sticky mobile CTA appears after module 3 with no animation.
**Focus and interaction states:** 2px oxblood outline with 2px offset on every interactive element; hover on buttons darkens oxblood 8%; links underline by default, no colour-only hover.
...
| Wordmark | Text "Firsthand" in Fraunces, small caps, letter-spaced | None; no logo file yet | Header, footer |
...
- Founder tasks in parallel, not blocking the build: two food-partner quotes; florist quote; waiver and commitment wording to a professional; a domain and a hello@ mailbox; Stripe account in test mode; USPTO search for Firsthand.

### Phase 1: Foundation
- `create-next-app` (TypeScript, Tailwind, App Router, src disabled to match CLAUDE.md's /app), on branch `staging`.
- Tokens, fonts, container, global semantics, focus styles, reduced-motion rule, `next/image` defaults.
...
**Task:** On branch `staging`, run `npx create-next-app@latest . --typescript --tailwind --app --no-src-dir --eslint --import-alias "@/*"` (confirm it does not clobber existing files; if it refuses because the directory is not empty, scaffold into a temp dir and move the generated files in). Enable `strict` and `noUncheckedIndexedAccess` in tsconfig. In app/globals.css define the `@theme` tokens from Section 2 (bone, ink, oxblood, warm-grey, rule, error, success) and the type scale as CSS custom properties. Load Fraunces (variable, axes opsz and wght) and Homemade Apple with `next/font/google` in app/layout.tsx and expose them as CSS variables `--font-serif` and `--font-hand`. Add a `.container` utility (max 1120px, 24px padding). Add global focus-visible styles (2px oxblood outline, 2px offset) and a `prefers-reduced-motion` rule that disables transitions. Install shadcn/ui and add Button, Input, Label, Textarea, RadioGroup; restyle Button to the oxblood spec. Add `@vercel/analytics` and mount `<Analytics />`. Set metadata: title "Firsthand · Her Night, Your Hands", description "One night at home, unmistakably yours. Dallas.", `robots: { index: false, follow: false }`.
...
**Review gate:** fonts render (Fraunces visible), tokens applied, Lighthouse accessibility 100 on the empty page.
...
**Task:** Build `components/site-header.tsx` (wordmark "Firsthand" in Fraunces small caps, letter-spaced, left; nothing else), `components/site-footer.tsx` (the footer line, links to /terms and /privacy, hello@ address from NEXT_PUBLIC_CONTACT_EMAIL or a TODO), and `components/hero.tsx`. Hero at desktop: 12-column grid; text in columns 1 to 5; image full-bleed from column 7 to the viewport edge, 3:2, `object-position: 20% center`. Mobile: image first at 4:5, then text. Text order: eyebrow "Firsthand · Dallas" (caption style), display-1 headline on three lines with `<br>` after "her." and "question.", the promise paragraph, the price paragraph, the primary Button "Hold a slot" (a form that posts to the Stripe Server Action from Prompt 4, stubbed for now), the secondary link "Not ready? Tell us about her." to `#tell-us`, then the qualifier line in caption style. Fire `track('cta_hold_slot_click', { location: 'hero' })` on click.
...
**Behavior and responsive rules:** hero image fades in 400ms, static under reduced motion; `priority` on the image; explicit width and height.
...
**Behavior and responsive rules:** `<ol>` with real numbers rendered via CSS counters in Fraunces.
...
**Do not:** add a timeline graphic; animate the steps on scroll.
**Output to show:** screenshots at 390 and 1280.
...
**Assets:** none; the card is markup.
**Visual rules:** this is the only element on the page with a border and radius; the handwritten face appears here for the second time; no drop shadow.
**Behavior and responsive rules:** rotation removed under 640px and under reduced motion.
**Accessibility requirements:** the card is a `<figure>` with `<figcaption>` "Card 4 of 12"; rotation does not affect reading order.
**Do not:** add a stack of multiple cards; add a "view all cards" link; photograph the card.
**Output to show:** screenshots at 390 and 1280.
**Review gate:** the card reads as an object, not a UI card; the handwritten face is used only on the title.
...
**Behavior and responsive rules:** `<dl>` single column on mobile.
...
**Behavior and responsive rules:** none.
...
**Behavior and responsive rules:** single column at all widths; pending state on submit; the sticky mobile CTA (a slim bar with "Hold a slot") appears after the user scrolls past Module 3 and hides when the form is in view.
...
**Accessibility requirements:** Lighthouse accessibility ≥ 95; no colour-only states; reduced motion honoured.
...
**Accessibility**
- Lighthouse accessibility ≥ 95; every interactive element keyboard reachable with a visible focus ring; labels on every field; errors announced; reduced motion honoured; contrast ≥ 4.5:1 for text (warm grey verified on bone).
